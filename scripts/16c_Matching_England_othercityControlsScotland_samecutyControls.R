...
#
# SAME-CITY EXCLUSION RULE:
#   Control OAs from the same LAD as a treated OA are excluded to avoid
#   indirect treatment contamination (traffic displacement, behavioural
#   spillover from the CAZ/LEZ). This rule is applied to ENGLAND ONLY.
#
#   In Scotland, this rule cannot be applied because all four major Scottish
#   cities (Glasgow, Edinburgh, Aberdeen, Dundee) implemented LEZs. Applying
#   the same-city exclusion removes every Scottish control OA, leaving 201
#   Scottish treated OAs with zero eligible comparators. Scottish controls
#   are therefore drawn from the same cities as Scottish treated OAs (outside
#   the buffer zone), which introduces a potential contamination risk.
#   This is acknowledged as a limitation of the Scottish analysis:
#     - Scottish controls may have experienced indirect effects of the LEZ
#       (e.g. reduced car travel into city centres suppressing feeder road
#       traffic), which would bias Scottish estimates toward zero
#     - Scottish results should therefore be interpreted with greater caution
#       than English results and treated as conservative lower-bound estimates
#   This is a real-world policy evaluation constraint caused by near-universal
#   LEZ adoption in Scotland, not a methodological choice.
#
# ANALYTICAL STRATEGY:
#   England = primary analysis (same-city exclusion applied; clean donor pool)
#   Scotland = secondary analysis (same-city exclusion not applied; potential
#              downward bias acknowledged; results reported separately)
#
# OUTPUTS:
...
# LADs containing treated OAs — used for same-city exclusion in England only
...
cat("English CAZ LADs (same-city exclusion applied):\n")
print(treated_lads_england)
cat("\nScottish LEZ LADs (same-city exclusion NOT applied — see header note):\n")
print(treated_lads_scotland)

# --- ENGLAND: same-city exclusion applied ------------------------------------
# control_group1 OAs from English treated LADs are removed to prevent
...
data_england <- OA_matching_dataset %>%
  filter(
    ...
    !(control_group1_OA == 1 & LAD24CD %in% treated_lads_england)  # same-city rule
  ) %>%
  mutate(treat_indicator = as.integer(treated_OA == 1))

cat("\n=== ENGLAND dataset (same-city exclusion applied) ===\n")
...
# --- SCOTLAND: same-city exclusion NOT applied --------------------------------
# All Scottish control OAs belong to LEZ LADs — applying the same-city rule
# leaves zero controls. Controls are therefore drawn from Scottish OAs outside
# the buffer zone, including those in LEZ cities. This is a known weakness:
# these controls may have experienced indirect LEZ effects, which would
# attenuate the estimated treatment effect for Scotland (bias toward zero).
# Scottish estimates should be read as conservative.
...
data_scotland <- OA_matching_dataset %>%
  filter(
    ...
    n_roads     >  0,
    !(treated_OA == 1 & zero_injury_OA == 1)
    # NOTE: same-city exclusion deliberately omitted for Scotland
    # See script header for full rationale
  ) %>%
  mutate(treat_indicator = as.integer(treated_OA == 1))

cat("=== SCOTLAND dataset (same-city exclusion NOT applied) ===\n")
...
cat("LIMITATION: Scottish controls include OAs from LEZ cities (outside buffer).\n")
cat("Potential indirect LEZ exposure may attenuate Scottish estimates.\n\n")

# Document the Scottish same-city OAs that are retained (would have been
# excluded in England) — useful for reporting in limitations section
scotland_samecity_controls <- data_scotland %>%
  filter(treat_indicator == 0,
         LAD24CD %in% treated_lads_scotland) %>%
  nrow()

cat("Scottish same-city controls retained (would be excluded under England rule):",
    scotland_samecity_controls, "\n\n")
...
# Transform each country dataset independently so winsorisation thresholds
...
select_ratio <- function(data, s2_vars, trend_vars, label,
  ...
  list(optimal_ratio = best$ratio, ratio_results = ratio_results,
       label = label)
}
...
ratio_sco <- select_ratio(s2_prep_sco$data, s2_prep_sco$s2_vars,
                          trend_vars_sco, "Scotland", 1:5)
# Scotland capped at 1:5 — pool is not thin in numbers but match quality
# deteriorates quickly because same-city controls share similar injury
# trends to treated OAs (both exposed to LEZ effects)
...
cat("Optimal ratio — Scotland: 1:", optimal_ratio_scotland,
    "(capped at 5; same-city controls used)\n\n")

# Save and plot ratio selection
ratio_combined <- bind_rows(
  ratio_eng$ratio_results %>% mutate(country = "England (same-city excluded)"),
  ratio_sco$ratio_results %>% mutate(
    country = "Scotland (same-city controls retained — limitation)")
)
...
p_ratio <- ratio_combined %>%
  ...
  scale_colour_manual(
    values = c(
      "England (same-city excluded)"                          = "#2E6FAB",
      "Scotland (same-city controls retained — limitation)"   = "#6B3FA0"
    )
  ) +
  ...
  labs(
    ...
    subtitle = paste0(
      ...
      "\nScotland uses same-city controls (outside buffer) — potential",
      " downward bias; see limitations"
    ),
    x       = "Matching ratio (1:k)",
    y       = "Maximum trend |SMD| after matching",
    colour  = NULL,
    caption = paste0(
      "Scotland same-city exclusion not applied: all Scottish control LADs",
      " are also treated LADs (near-universal LEZ adoption).\n",
      "Scottish estimates may be attenuated if controls were indirectly",
      " affected by the LEZ. Results reported separately."
    )
  ) +
...
isolated_combined <- bind_rows(
  ...
  result_scotland$isolated_OAs %>% mutate(
    country = "Scotland",
    note    = "Same-city controls used — potential downward bias"
  )
)
...
cat("Scottish estimates may be attenuated. Report England and Scotland\n")
cat("results separately and flag this limitation explicitly.\n")
...
cat("  OA_matched_full_mixed_Scotland.rds— Scotland (same-city controls)\n")
