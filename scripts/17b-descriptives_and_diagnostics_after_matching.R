...
#   OA_common_support_flags.rds
#   OA_matching_census.rds
#   OA_s2_England.rds
#
# SPATIAL INPUTS (Section 15 — maps):
...
keep_england <- function(d) {
  if ("country" %in% names(d)) {
    d %>% filter(country == "England" | is.na(country))
  } else {
    d
  }
}
...
treated_England  <- readRDS(here("data", "processed", "OA_matched_treated_England.rds"))
controls_England <- readRDS(here("data", "processed", "OA_matched_donors_England.rds"))
...
s2_England       <- readRDS(here("data", "processed", "OA_s2_England.rds"))

treated_England  <- keep_england(treated_England)
controls_England <- keep_england(controls_England)
full_data        <- keep_england(full_data)

matched_England <- bind_rows(
  treated_England %>%
    mutate(treat_indicator = 1),
  controls_England %>%
    mutate(treat_indicator = 0)
) %>%
  left_join(
    full_data |> dplyr::select(-any_of(c("treat_indicator", "treated_OA",
                                         "buffer_OA", "control_group1_OA",
                                         "control_group2_OA", "country"))),
    by = "OA"
  )

# Compute per-treated-OA mean Mahalanobis distance from the stage-2 matchit object
local({
  m    <- s2_England$matchit_obj
  X    <- as.matrix(m$X)
  S_inv <- MASS::ginv(cov(X))
  mm   <- m$match.matrix                        # rows = treated, cols = control indices
  oa_all <- s2_England$primary_data$OA

  mdist_tbl <- purrr::map_df(seq_len(nrow(mm)), function(i) {
    ctrl_idx <- as.integer(mm[i, ])
    ctrl_idx <- ctrl_idx[!is.na(ctrl_idx)]
    xi  <- X[i, , drop = FALSE]
    xc  <- X[ctrl_idx, , drop = FALSE]
    d   <- apply(xc, 1, function(xj) {
      diff <- xi - xj
      sqrt(as.numeric(diff %*% S_inv %*% t(diff)))
    })
    tibble::tibble(OA = oa_all[i], mdist = mean(d))
  })

  matched_England <<- matched_England %>%
    left_join(mdist_tbl, by = "OA")
})
...
stage1_road <- c(
  "log1p_road_length_km", "log1p_road_density_m_km2",
  "log_area_km2",
...
desc_treated_pre <- desc_stats(
  unmatched_pool %>% filter(treat_indicator == 1),
  all_desc_vars,
  "Treated"
)

desc_control_pre <- desc_stats(
  unmatched_pool %>% filter(treat_indicator == 0),
  all_desc_vars,
  "Unmatched controls"
)

desc_control_post <- desc_stats(
  matched_England %>% filter(treat_indicator == 0),
  all_desc_vars,
  "Matched controls"
)
...
smd_s1 <- smd_table(stage1_vars, unmatched_pool, matched_England) %>%
  rename(
    smd_unmatched = smd_before,
...
smd_s2 <- smd_table(c(stage2_trends, stage2_levels), unmatched_pool, matched_England) %>%
  rename(
...
isolated_ids <- csupport$treated_OA[csupport$structurally_isolated]

matched_England_treated <- matched_England %>%
  filter(treat_indicator == 1)

isolated_chars <- matched_England_treated %>%
...
matched_England_treated_flag <- matched_England_treated %>%
...
smd_isolated <- map_df(stage1_vars, function(v) {
  tibble(
    variable = v,
    label = coalesce(var_labels[v], v),
    smd = round(compute_smd(matched_England_treated_flag, v), 3)
  )
}) %>%
...
ld_s1 <- love_data_fn(unmatched_pool_log, matched_England, stage1_vars)

p_love_s1 <- make_love_plot(
  ld_s1,
  "Stage 1 Balance",
  "Structural and sociodemographic variables | ratio 10"
)

save_fig(p_love_s1, "fig01_love_plot_stage1_England.png", width = 16, height = 18)

ld_s2 <- love_data_fn(unmatched_pool, matched_England, c(stage2_trends, stage2_levels))

p_love_s2 <- make_love_plot(
  ld_s2,
  "Stage 2 Balance",
  "Pre-treatment injury trends and levels | ratio 3"
)

save_fig(p_love_s2, "fig02_love_plot_stage2_England.png", width = 16, height = 16)

cat("  Saved: fig01_love_plot_stage1_England.png\n")
cat("  Saved: fig02_love_plot_stage2_England.png\n\n")
...
specs_heat <- list(
  ...
  "After Stage 1" = matched_England,
  "After Stage 2" = matched_England
)
...
p_heatmap <- ggplot(smd_heat_data, aes(x = spec, y = label, fill = smd)) +
  ...
  labs(
    title = "Absolute SMD Across Specifications",
...
save_fig(p_heatmap, "fig03_smd_heatmapEngland.png", width = 14, height = 20)
...
road_plots <- map(c(dist_road_vars, dist_urban_vars), function(v) {
  plot_density_pair(v, unmatched_pool, matched_England, log_scale = v %in% log_s1)
})
...
save_fig(p_dist_road, "fig04a_dist_road_networkEngland.png", width = 16, height = 22)

socdem_plots <- map(dist_socdem_vars, function(v) {
  plot_density_pair(v, unmatched_pool, matched_England, log_scale = FALSE)
})
...
save_fig(p_dist_socdem, "fig04b_dist_socdemEngland.png", width = 16, height = 24)

key_trends <- c(
  "trend_total_pkm", "trend_ped_slight_pkm",
  "trend_car_slight_pkm", "trend_cyc_slight_pkm"
)

trend_plots <- map(key_trends, function(v) {
  plot_density_pair(v, unmatched_pool, matched_England)
})
...
save_fig(p_dist_trends, "fig05_dist_stage2_trendsEngland.png", width = 14, height = 10)
...
ctrl_A <- matched_England %>%
  filter(treat_indicator == 0)

eff_n_overall <- round(sum(ctrl_A$weights)^2 / sum(ctrl_A$weights^2), 0)

p_w1 <- ggplot(ctrl_A, aes(x = weights)) +
  geom_histogram(bins = 60, fill = COL_CONTROL, alpha = 0.85) +
  scale_x_continuous(limits = c(0, 5.5)) +
  labs(
    title = "Control OA Weight Distribution",
    subtitle = paste0(
      "Nominal N = ", nrow(ctrl_A),
      " | Effective N = ", eff_n_overall,
      " | Efficiency = ", round(eff_n_overall / nrow(ctrl_A), 3)
    ),
    x = "Weight",
    y = "Count"
  ) +
...
p_w2 <- tibble(
  ...
  n = nrow(ctrl_A),
  eff_n = round(sum(ctrl_A$weights)^2 / sum(ctrl_A$weights^2), 1),
  max_w = round(max(ctrl_A$weights), 2)
) %>%
  ggplot(aes(x = group, y = eff_n)) +
  geom_col(width = 0.45, fill = COL_CONTROL) +
  geom_text(aes(label = paste0("n=", n, "\neff_N=", eff_n, "\nmax_w=", max_w)),
            vjust = -0.2, size = 3.5, lineheight = 1.1) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.25))) +
  labs(title = "Effective N", x = NULL, y = "Effective N") +
  theme_diag()

p_w3 <- ggplot(ctrl_A, aes(x = weights)) +
  stat_ecdf(linewidth = 1, colour = COL_CONTROL) +
  geom_vline(xintercept = 5, linetype = "dashed",
             colour = "#E74C3C", linewidth = 0.6) +
  coord_cartesian(xlim = c(0, 6)) +
  labs(
    title = "ECDF of weights",
...
p_weights <- (p_w1 | p_w2 | p_w3) +
  plot_annotation(
    ...
    theme = theme(
      plot.title = element_text(size = 13, face = "bold", colour = "#1A2E5A")
    )
  )

save_fig(p_weights, "fig06_weight_diagnosticsEngland.png", width = 16, height = 6)
cat("  Saved: fig06_weight_diagnosticsEngland.png\n\n")

control_reuse <- matched_England %>%
  filter(treat_indicator == 0) %>%
...
mdist_A <- matched_England %>%
  filter(treat_indicator == 1, !is.na(mdist)) %>%
  select(OA, mdist)

p_mdist_ecdf <- ggplot(mdist_A, aes(x = mdist)) +
  stat_ecdf(linewidth = 1.1, colour = COL_CONTROL) +
  ...
  labs(
    ...
    caption = paste0(
      "n = ", nrow(mdist_A), " treated OAs | ",
      sum(mdist_A$mdist > 20), " OAs with distance > 20"
    )
  ) +
  theme_diag()

p_mdist_hist <- ggplot(mdist_A, aes(x = mdist)) +
  geom_histogram(bins = 40, alpha = 0.75, fill = COL_CONTROL) +
  coord_cartesian(xlim = c(0, 35)) +
  labs(
    title = "Stage 2 Distance Distribution",
...
save_fig(p_mdist, "fig07_mahalanobis_distanceEngland.png", width = 12, height = 11)
...
treated_quartiles <- matched_England |>
  filter(treat_indicator == 1, !is.na(mdist)) |>
  mutate(
    dist_quartile = ntile(mdist, 4),
    dist_quartile_label = factor(
      quartile_labels[dist_quartile],
      levels = quartile_labels
    )
  )

mm_s2 <- s2_England$matchit_obj$match.matrix
...
pair_list <- map_df(seq_len(nrow(mm_s2)), function(i) {
  ...
  q <- treated_quartiles$dist_quartile[
    match(t_row, which(matched_England$treat_indicator == 1))
  ]
  ...
  tibble(
    row_idx = c(t_row, c_rows),
    dist_quartile = q,
    dist_quartile_label = q_lbl
  )
})

matched_with_q <- matched_England |>
...
smd_q_vars <- intersect(smd_q_vars, names(matched_England))

smd_by_quartile <- map_df(1:4, function(q) {
  d <- matched_with_q |>
    filter(dist_quartile == q)
  ...
  mutate(
    var_type = if_else(
      variable %in% stage2_trends,
      "Stage 2 trend variable",
      "Stage 1 structural variable"
    )
  )
...
p_smd_quartile <- ggplot(
  ...
  labs(
    title = "Residual Imbalance by Mahalanobis Distance Quartile",
    subtitle = paste0(
      "Each line = one variable | Dashed = |SMD| = 0.10 threshold\n",
      q_sizes
    ),
    x = "Distance quartile (Stage 2 Mahalanobis distance)",
    y = "Absolute SMD",
    caption = "Rising lines toward Q4 indicate that residual imbalance is concentrated in poorly matched OAs."
  ) +
  theme_diag() +
  theme(
...
save_fig(p_smd_quartile, "fig07b_smd_by_distance_quartile_England.png", width = 16, height = 14)

# =============================================================================
# SECTION 13 — STRATUM INJURY DISTRIBUTION
# =============================================================================

cat("=== Section 13: Stratum injury distribution ===\n")

strat_data <- matched_England %>%
...
p_strat_inj <- ggplot(strat_data, aes(x = stratum, y = mean_total_pkm, fill = stratum)) +
...
p_strat_covs <- strat_data %>%
  select(
    stratum, Drive_Car_pct, Walk_pct, IMD, pop_density,
...
save_fig(p_strat_inj, "fig08_stratum_injury_England.png", width = 10, height = 7)
save_fig(p_strat_covs, "fig09_stratum_covariates_England.png", width = 13, height = 10)
...
if (!file.exists(oa_path) || !file.exists(lad_path)) {
  ...
  if (!is.na(lad_id_col)) {
    lad_sf <- lad_sf %>%
      rename(LAD_CODE = all_of(lad_id_col))
  }
  
  if (!is.na(lad_nm_col)) {
    lad_sf <- lad_sf %>%
      rename(LAD_NAME = all_of(lad_nm_col))
  }
  ...
  map_status <- full_data %>%
    mutate(
      map_group = case_when(
        ...
        OA %in% controls_England$OA ~ "Matched control",
        control_group1_OA == 1 | control_group2_OA == 1 ~ "Eligible (unmatched)",
        TRUE ~ "Other"
      )
    ) %>%
    select(OA, map_group, LAD24CD)
  
  treated_centroids <- oa_sf %>%
    filter(OA %in% treated_England$OA) %>%
    st_centroid()
  
  p_overview <- ggplot() +
    geom_sf(data = lad_sf, fill = "#F5F7FA", colour = "#CCCCCC", linewidth = 0.2) +
    geom_sf(data = treated_centroids, colour = COL_TREATED, size = 1.2, alpha = 0.7) +
    coord_sf(xlim = c(-6, 2), ylim = c(50, 56.5), crs = 4326) +
    labs(
      title = "Geographic Distribution of Treated OAs",
      subtitle = paste0(nrow(treated_centroids), " treated OAs"),
  ...
  save_fig(p_overview, "fig10_map_treated_overviewEngland.png", width = 8, height = 12)
  
  controls_per_lad <- full_data %>%
    filter(OA %in% controls_England$OA) %>%
  ...
  save_fig(p_control_origin, "fig11_map_control_originEngland.png", width = 9, height = 13)
  ...
  walk(top_cities, function(lad_cd) {
    ...
    p_city <- ggplot() +
      ...
      labs(
        title = paste("Treated and Matched Control OAs —", city_nm),
        subtitle = paste0(n_t, " treated OAs | ", n_c, " matched controls"),
        caption = "Blue = matched controls | Orange = buffer | Red = treated | Light blue = eligible unmatched"
      ) +
      theme_diag() +
      theme(
        axis.text = element_blank(),
    ...
    fn <- paste0("fig12_map_city_England", gsub("[^A-Za-z0-9]", "_", lad_cd), ".png")
...
# =============================================================================
# SECTION 15 — PARALLEL TRENDS
# =============================================================================

cat("=== Section 15: Parallel trends visualisation (slope distributions) ===\n")

trend_long <- matched_England %>%
...
p_trend_density <- ggplot(trend_long, aes(x = slope, colour = group, fill = group)) +
  ...
  labs(
    ...
    fill = NULL,
    caption = paste0(
      "n treated = ", sum(matched_England$treat_indicator == 1),
      " | n controls = ", sum(matched_England$treat_indicator == 0),
...
save_fig(p_trend_density, "fig14_parallel_trends_slopesEngland.png", width = 14, height = 12)
...
cat("SAMPLE SIZES:\n")
cat(sprintf("  Treated:  %d\n", sum(matched_England$treat_indicator == 1)))
cat(sprintf("  Controls: %d\n", sum(matched_England$treat_indicator == 0)))
cat(sprintf(
  "  Isolated OAs (flagged, included in this analysis): %d\n",
  length(isolated_ids)
))
