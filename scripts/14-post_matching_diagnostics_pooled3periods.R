...
stratum_vars <- intersect(
  c("mean_precovid_total_pkm", "road_length_km", "road_density_m_km2",
...
if (n_isolated > 0) {
  ...
  iso_compare <- matched_full %>%
    ...
    summarise(
      n = n(),
      across(all_of(intersect(c("mean_precovid_total_pkm", "road_length_km",
...
if (length(trend_vars) > 0) {
  ...
  p_trends <- ggplot(trend_data, aes(x = value, fill = group, colour = group)) +
    ...
    labs(
      title    = "Stage 2 trajectory distributions: treated vs matched controls",
  ...
  p_trends_scheme <- ggplot(trend_scheme_data,
    ...
    scale_colour_manual(values = c("Treated" = COL_TREATED, "Control" = COL_CONTROL)) +
    labs(
      title    = "Stage 2 trajectory distributions by scheme: treated vs matched controls",
      subtitle = "Parsimonious matching variables: overall trend, COVID response, recovery response",
...
# Stage 2 variables used in the parsimonious matching distance + key Stage 1 variables
...
p_love_scheme <- ggplot(
  ...
  labs(
    title    = "Balance by scheme: parsimonious Stage 2 + key covariates",
    subtitle = "Before vs after matching; Stage 2 matches trajectory, pre-COVID level, and zero-quarter sparsity",
    x = "|SMD|", y = NULL, colour = NULL, shape = NULL,
    caption = "Dashed = 0.10 threshold"
  ) +
  theme_diag() +
