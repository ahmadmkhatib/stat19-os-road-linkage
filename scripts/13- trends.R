...
quarterly_trends <- road_panel_model %>%
  ...
  mutate(
    quarter_date   = as.Date(zoo::as.yearqtr(quarter_year, format = "%Y Q%q")),
...
p <- ggplot(
  ...
  # ── 2. LOESS trend lines per group × period segment ───────────────────────
  geom_smooth(
  ...
  labs(
    ...
    subtitle = paste0(
      ...
      "   |   ",
      "Dashed lines = LOESS trend per period"
    ),
    caption  = paste0(
      "Periods: ",
      "(1) Pre-COVID  |  ",
...
table(road_panel_model$group)

road_panel_model <- road_panel_model %>%  
  mutate(scheme=if_else(is.na(scheme), "Outside_CAZ_areas", scheme))
...
quarterly_trends <- road_panel_model %>%
  ...
  summarise(
    total_injuries =
      sum(across(starts_with("total_inj_adj")), na.rm = TRUE),
    .groups = "drop"
  )
table(quarterly_trends$group)
...
# plot
ggplot(quarterly_trends,
       aes(x = quarter_year, y = total_injuries, colour = group)) +
  geom_line() +
  facet_wrap(~scheme, scales = "free_y") +
  theme_minimal() +
  labs(
    title = "Quarterly Road Injury Trends inside and outside CAZ areas",
    x = "Quarter",
    y = "Total Injuries",
    colour = "Road group"
  )
...
# Add a column indicating before/after CAZ for each scheme
quarterly_trends <- quarterly_trends %>%
  left_join(road_caz_prop %>% select(scheme, caz_start_date), by = "scheme") %>%
  mutate(period = case_when(
    quarter_date < caz_start_date ~ "pre_CAZ",
    quarter_date >= caz_start_date ~ "post_CAZ"
  ))



road_caz_prop <- road_caz_prop %>%
  mutate(vline_label = "CAZ start date")
         
         
         
# Plot
ggplot(quarterly_trends, aes(x = quarter_date, y = total_injuries, color = group)) +
  ...
  # Trend lines per group, split by period
  geom_smooth(aes(group = interaction(group, period)), 
              method = "loess", se = FALSE, linetype = "dashed", color = "blue") +
  
  # Observed data lines
  geom_line(aes(group = group)) +
  
  # CAZ implementation vertical line
  geom_vline(
    data = road_caz_prop,
    aes(xintercept = as.numeric(caz_start_date), linetype = vline_label),
  ...
  labs(
    title = "Quarterly Road Injury Trends by CAZ areas",
    x = "Quarter",
    y = "Total Injuries",
    color = "Road group"
  ) +
