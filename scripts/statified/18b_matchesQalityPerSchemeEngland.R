...
oa_panel <- oa_q %>%
  inner_join(
    matched_full %>%
      select(OA, treat_indicator, weights, scheme, country, road_length_km),
    by = "OA"
  ) %>%
  left_join(scheme_starts, by = "scheme") %>%
  # Per-km injury rate (avoid division by zero)
...
facet_info <- trend_rank %>%
  ...
  mutate(
    facet_label = paste0(rank, ". ", scheme),
...
rank_bar_data <- trend_rank %>%
  mutate(
    ...
    scheme_lab = fct_reorder(scheme, -mean_abs_smd)
  )
...
#
# Interpretation:
#   NMAG = 0  → trajectories perfectly overlap
#   NMAG = 1  → average gap equals the pooled mean (trajectories diverge
#                by ~100% of the baseline rate)
#
# Unlike the SMD ranking (which summarises balance in pre-treatment trend
# *slopes*), this metric captures the actual time-series alignment —
# including level shifts, differential seasonality, and within-period
...
trajectory_gap <- oa_pre_quarterly %>%
  ...
  summarise(
    ...
    pooled_mean = mean((Treated + `Matched control`) / 2, na.rm = TRUE),
    norm_gap    = mean_gap / pooled_mean,
    .groups     = "drop"
  )

traj_breaks  <- c(0, 0.25, 0.40, 0.60, Inf)
traj_labels  <- c("Strong (<0.25)", "Acceptable (0.25\u20130.40)",
                   "Marginal (0.40\u20130.60)", "Poor (>0.60)")
traj_colours <- c(
  "Strong (<0.25)"               = "#1a9850",
  "Acceptable (0.25\u20130.40)"  = "#91cf60",
  "Marginal (0.40\u20130.60)"    = "#fc8d59",
  "Poor (>0.60)"                 = "#d73027"
)

traj_rank_data <- trajectory_gap %>%
  left_join(
    matched_full %>%
      filter(treat_indicator == 1) %>%
      distinct(scheme, country),
  ...
  mutate(
    quality = cut(norm_gap, breaks = traj_breaks,
                  labels = traj_labels, right = FALSE, include.lowest = TRUE),
    scheme_lab = fct_reorder(scheme, -norm_gap)
  )

p_traj_ranking <- ggplot(
  traj_rank_data,
  aes(y = scheme_lab, x = norm_gap, fill = quality)
) +
  geom_col(width = 0.7, alpha = 0.9) +
  geom_vline(xintercept = 0.25, linetype = "dashed",
             colour = "#555555", linewidth = 0.5) +
  geom_text(aes(label = sprintf("%.3f", norm_gap)),
            hjust = -0.15, size = 3.2, colour = "#333333", fontface = "bold") +
...
save_fig(p_traj_ranking, "fig_parallel_trends_trajectory_ranking.png",
         width = 10, height = 7)

cat("\nParallel trends outputs saved:\n")
cat("  fig_parallel_trends_timeseries.png\n")
cat("  fig_parallel_trends_ranking.png\n")
cat("  fig_parallel_trends_trajectory_ranking.png\n")
cat("================================================================\n")
