  ...
  #   England: other-city controls only, per-scheme matching
  ...
  cat("  Treated:", sum(matched_A$treat_indicator == 1),
      "| Control rows:", sum(matched_A$treat_indicator == 0),
      "| Unique control OAs:", n_distinct(matched_A$OA[matched_A$treat_indicator == 0]),
      "\n\n")
  ...
  # Per-scheme matched sample breakdown
  # S2 ratio derived from matching pairs (max controls per treated OA per scheme).
  # Unique controls can be < treated × ratio due to replacement reuse.
  cat("=== Per-scheme matched sample ===\n")

  s2_ratio_per_scheme <- pairs_mixed %>%
    count(scheme, treated_OA) %>%
    group_by(scheme) %>%
    summarise(s2_ratio = paste0("1:", max(n)), .groups = "drop")

  pairs_mixed %>%
    group_by(scheme) %>%
    summarise(
      treated         = n_distinct(treated_OA),
      total_pairs     = n(),
      unique_controls = n_distinct(control_OA),
      .groups         = "drop"
    ) %>%
    left_join(s2_ratio_per_scheme, by = "scheme") %>%
    print()

  cat("\nTotal unique control OAs (across all schemes):",
      n_distinct(matched_A$OA[matched_A$treat_indicator == 0]),
      "(some controls serve multiple schemes)\n\n")
  ...
  p_love_s1_combined <- make_love_plot(
    ...
    "Structural & sociodemographic variables (log-scale where matched) | per-scheme matching, S1 ratio 1:50")
  p_love_s1_eng <- make_love_plot(
    ld_s1_eng,
    "Stage 1 Balance \u2014 England",
    "Structural & sociodemographic variables (log-scale where matched) | per-scheme matching, S1 ratio 1:50, other-city controls")
  ...
  p_love_s2_eng <- make_love_plot(
    ...
    "Injury trends (raw) + mean levels (log1p) \u2014 both on matching scale | per-scheme S2 ratio varies")
