...
counts <- list(
  oa_roads=73483, matching_total=75275, matching_eng=57805, matching_sco=17470,
  treated_all=1066, buffer_all=3493, ctrl1_all=19395, ctrl2_all=51321,
  # Sequential non-overlapping exclusion cascade (75,275 → 61,687)
  excl_total=13588,
  excl_buffer=3493, eng_buffer=1597, sco_buffer=1896,
  excl_eng_samecity=8421, excl_eng_ctrl=10018,
  excl_zero_roads=1483, excl_zero_roads_pct=2.3,
  excl_zero_inj_treated=191, excl_zero_road_treated=61, excl_treated_total=252,
  post_buffer=71782, post_samecity=63361, post_roads=61878,
  eligible=61687, elig_eng=46831, elig_sco=14856, elig_treated=814,
  elig_ctrl=60873, elig_eng_treated=613, elig_sco_treated=201,
  elig_eng_ctrl=46218, elig_sco_ctrl=14655,
  s1_total=4677, s1_treated=814, s1_ctrl=3863,
  s1_eng=3800, s1_eng_treated=613, s1_eng_ctrl=3187,
  s1_sco=877,  s1_sco_treated=201, s1_sco_ctrl=676,
  s1_ctrl_excl=57010, s2_ctrl_excl=2864,
  final_total=1813, final_treated=814, final_ctrl=999,
  final_eng=1487, final_eng_treated=613, final_eng_ctrl=874,
  final_sco=326,  final_sco_treated=201, final_sco_ctrl=125,
  isolated_total=41, isolated_eng=31, isolated_sco=10
)
...
txt <- list(
  ...
  b2_body = paste0("N = ", fN(counts$matching_total), " OAs\n",
                   "England: ", fN(counts$matching_eng),
                   "   Scotland: ", fN(counts$matching_sco), "\n",
                   "Treated: ", fN(counts$treated_all),
                   "   Buffer: ", fN(counts$buffer_all), "\n",
                   "Same-city controls: ", fN(counts$ctrl1_all),
                   "   Other-city: ", fN(counts$ctrl2_all)),

  b3_head = "Eligible Study Population",
  b3_body = paste0("N = ", fN(counts$eligible), "\n",
                   "England: ", fN(counts$elig_eng),
                   "   Scotland: ", fN(counts$elig_sco), "\n",
                   "Treated: ", fN(counts$elig_treated),
                   "  (England: ", fN(counts$elig_eng_treated),
                   ";  Scotland: ", fN(counts$elig_sco_treated), ")\n",
                   "Controls: ", fN(counts$elig_ctrl),
                   "  (Eng: ", fN(counts$elig_eng_ctrl),
                   ";  Sco: ", fN(counts$elig_sco_ctrl), ")"),

  b4_head = "Stage 1: Structural control Pool Restriction",
  b4_body = paste0("N = ", fN(counts$s1_total), " OAs\n",
                   "England: ", fN(counts$s1_eng),
                   "  (", fN(counts$s1_eng_treated),
                   " treated; ", fN(counts$s1_eng_ctrl), " controls)\n",
                   "Scotland: ", fN(counts$s1_sco),
                   "  (", fN(counts$s1_sco_treated),
                   " treated; ", fN(counts$s1_sco_ctrl), " controls)\n",
                   "Total: ", fN(counts$s1_treated),
                   " treated   ", fN(counts$s1_ctrl), " controls"),

  # Combined Stage 2 + Analytic Sample box
  ...
  b4b_body = paste0("N = ", fN(counts$final_total), " OAs\n",
                    ...
                    "England: ", fN(counts$final_eng),
                    "  (", fN(counts$final_eng_treated),
                    " treated; ", fN(counts$final_eng_ctrl), " controls)\n",
                    "Scotland: ", fN(counts$final_sco),
                    "  (", fN(counts$final_sco_treated),
                    " treated; ", fN(counts$final_sco_ctrl), " controls)"),
  b4b_foot = "Analytical Sample",

  eA_head = paste0("Excluded:  ", fN(counts$excl_total), " OAs"),
  eA_body = paste0(
    "Buffer (Eng: ", fN(counts$eng_buffer),
    ";  Sco: ", fN(counts$sco_buffer), ")",
    "  \u2192  ", fN(counts$post_buffer), "\n",
    "Eng. same-city (CAZ): ", fN(counts$excl_eng_samecity),
    "  \u2192  ", fN(counts$post_samecity), "\n",
    "Zero roads (", counts$excl_zero_roads_pct, "%): ",
    fN(counts$excl_zero_roads),
...
# Sizing constants
LH  <- 4.0   # vertical space per line of text
PAD <- 3.5   # vertical padding inside box (top & bottom)
GAP <- 2.5   # gap between boxes / tee arms
...
# =============================================================================
# LAYOUT  — exclusion boxes hang below each box's bottom edge
# =============================================================================
CX  <- 34;  BX1 <- 1;  BX2 <- 67     # main boxes
ECX <- 84;  EX1 <- 70; EX2 <- 98     # exclusion boxes

# Vertical gap between boxes must fit the hanging exclusion box
gap_A <- eA_h + 2 * GAP   # space below B2 for eA
gap_B <- eB_h + 2 * GAP   # space below B4 for eB
...
# Exclusion boxes hang directly below their source box
...
fill_final <- "#E4F2E4";  fill_excl  <- "#FDF0ED";  fill_flag  <- "#FFF8E1"
brd_data   <- "#4A7FB5";  brd_elig   <- "#2E6FAB";  brd_stage  <- "#6B3FA0"
brd_final  <- "#2A7A2A";  brd_excl   <- "#C0392B";  brd_flag   <- "#D4900A"
col_arrow  <- "#444444";  col_title  <- "#1A2E5A"

# Font sizes (ggplot2 mm → approx pt:  size 8 ≈ 22pt,  size 9 ≈ 25pt)
fH <- 9      # box headers
fB <- 8      # box body text
fI <- 7      # italic notes
fS <- 8.5    # final box sub-header
fE <- 8      # exclusion header
fe <- 7      # exclusion body
...
p <- ggplot() +
  ...
  # plain arrow OUT of B3 bottom (no exclusion at this step)
  ...
  labs(
    ...
    subtitle = "Output Areas (OAs)·  England and Scotland",
    caption  = "CAZ = Clean Air Zone (England)  ·  LEZ = Low Emission Zone (Scotland)"
  ) +
...
ggsave(file.path(outdir, "fig15_sample_flow_diagram.png"),
       plot = p, width = 22, height = 32, dpi = 300, bg = "white")
cat("Saved:", file.path(outdir, "fig15_sample_flow_diagram.png"), "\n")
